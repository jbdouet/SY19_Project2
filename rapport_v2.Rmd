...
load("~/SY19/SY19_Project2/envJB.RData")
library(caret)
library(car)
library("e1071")
...
Pour les courbes affichées nous avons choisi 25 composantes principales ce qui visuellement et après plusieurs tests nous est apparu comme le meilleur compromis.
...
Différentes combinaison parmi ces étapes ont donc été testées en fonction des propriétés des classifieurs.

## Analyse
Voici les résultats (la précision obtenue) pour les differentes méthodes testées.
Au niveau du preprocessing, pour toutes les méthodes, les pixels noirs ont été retirés. 
Les autres étapes ont été testées en même temps que les classifieurs comme cela:
  ...
  * RDA + Sélection des zones expressives + PCA :0.78
  * RandomForest + Sélection des zones expressives + PCA : 0.68
  * XGBOOST+ Sélection des zones expressives + PCA: 0.71

## Observations 

Globalement, la sélections des zones expressives du visage couplée au PCA améliore les performances par rapport au PCA normal. A l'exception du RDA (regularized discriminant analysis) qui a de meilleures performances lorsque seul le PCA est appliqué.
On notera que les performances algorithmes de SVM linéaire et de random forest 
...
Malheureusement, avec 108 individus, même un réseau de neurones peu profond ne converge pas. 
